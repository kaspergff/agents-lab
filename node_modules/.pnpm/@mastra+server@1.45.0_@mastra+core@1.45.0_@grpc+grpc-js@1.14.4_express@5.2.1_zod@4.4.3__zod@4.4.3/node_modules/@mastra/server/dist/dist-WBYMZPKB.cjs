              ...
              parts.push(
                part.image instanceof URL ? {
                  ...
                  inlineData: {
                    ...
                    data: chunkFEUXFWF5_cjs.convertUint8ArrayToBase64(part.image)
                  }
                }
              );
              break;
...
var googleFailedResponseHandler = chunkFEUXFWF5_cjs.createJsonErrorResponseHandler({
...
var GoogleGenerativeAILanguageModel = class {
  ...
  async getArgs({
    ...
    const warnings = [];
    const googleOptions = chunkFEUXFWF5_cjs.parseProviderOptions({
      provider: "google",
  ...
  async doGenerate(options) {
    ...
    const mergedHeaders = chunkFEUXFWF5_cjs.combineHeaders(
      await chunkFEUXFWF5_cjs.resolve(this.config.headers),
      options.headers
    );
    const {
      responseHeaders,
      ...
      successfulResponseHandler: chunkFEUXFWF5_cjs.createJsonResponseHandler(responseSchema),
  ...
  async doStream(options) {
    const { args, warnings } = await this.getArgs(options);
    const body = JSON.stringify(args);
    const headers = chunkFEUXFWF5_cjs.combineHeaders(
      await chunkFEUXFWF5_cjs.resolve(this.config.headers),
      options.headers
    );
    const { responseHeaders, value: response } = await chunkFEUXFWF5_cjs.postJsonToApi({
      ...
      body: args,
      failedResponseHandler: googleFailedResponseHandler,
      successfulResponseHandler: chunkFEUXFWF5_cjs.createEventSourceResponseHandler(chunkSchema),
      abortSignal: options.abortSignal,
      fetch: this.config.fetch
    });
    const { contents: rawPrompt, ...rawSettings } = args;
...
function createGoogleGenerativeAI(options = {}) {
  var _a;
  const baseURL = (_a = chunkFEUXFWF5_cjs.withoutTrailingSlash(options.baseURL)) != null ? _a : "https://generativelanguage.googleapis.com/v1beta";
  const getHeaders = () => ({
    "x-goog-api-key": chunkFEUXFWF5_cjs.loadApiKey({
  ...
  const createChatModel = (modelId, settings = {}) => {
    ...
    return new GoogleGenerativeAILanguageModel(modelId, settings, {
      ...
      baseURL,
      headers: getHeaders,
      generateId: (_a2 = options.generateId) != null ? _a2 : chunkFEUXFWF5_cjs.generateId,
...
//# sourceMappingURL=dist-WBYMZPKB.cjs.map
//# sourceMappingURL=dist-WBYMZPKB.cjs.map