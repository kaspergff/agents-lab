...
var chunkOEW7GS6U_cjs = require('./chunk-OEW7GS6U.cjs');
...
function captureLicenseCheck({
  ...
  const license = chunkTKFSM674_cjs.getSafeLicenseSummary();
  try {
    const ip = getRequestIp(request);
    chunkOEW7GS6U_cjs.captureEEEvent("ee_license_check", user?.id || license.anonymousId || chunkOEW7GS6U_cjs.getEETelemetryFallbackDistinctId(), {
...
async function buildCapabilities(auth, request, options) {
  ...
  const hasLicense = chunkTKFSM674_cjs.isLicenseValid();
  const isCloud = isMastraCloudAuth(auth);
  const isSimple = isSimpleAuth(auth);
  const isDev = chunkTKFSM674_cjs.isDevEnvironment();
  if (isDev && !hasLicense) {
    chunkTKFSM674_cjs.warnIfDevEENeedsLicense();
  }
  const isLicensedOrCloud = hasLicense || isCloud || isSimple || isDev;
  const isFeatureLicensed = (feature) => isCloud || isSimple || isDev || hasLicense && chunkTKFSM674_cjs.isFeatureEnabled(feature);
  let login = null;
  ...
  if (hasRBAC && rbacProvider) {
    try {
      ...
      try {
        const ip = getRequestIp(request);
        chunkOEW7GS6U_cjs.captureEEEvent("ee_feature_used", user.id || license.anonymousId || chunkOEW7GS6U_cjs.getEETelemetryFallbackDistinctId(), {
          feature: "rbac",
          user_id: user.id,
...
//# sourceMappingURL=chunk-GBRVBJXF.cjs.map
//# sourceMappingURL=chunk-GBRVBJXF.cjs.map