...
function splitRuntimeModelString(input) {
  const providers = chunkDURT53SS_cjs.getRegisteredProviders().sort((a, b) => b.length - a.length);
  ...
  const parsed = chunkDURT53SS_cjs.parseModelString(input);
...
function isModelAllowed(allowed, candidate) {
  ...
  const activeEntries = allowed.filter((entry) => {
    ...
    return chunkDURT53SS_cjs.isProviderRegistered(entry.provider);
  });
