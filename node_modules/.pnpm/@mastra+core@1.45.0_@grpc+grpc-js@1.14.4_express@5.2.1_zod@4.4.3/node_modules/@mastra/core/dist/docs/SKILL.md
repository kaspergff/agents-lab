...
- [Modes](references/docs-harness-modes.md) - Configure and switch between agent modes to give a single Harness multiple personalities or capability sets.
- [Harness overview](references/docs-harness-overview.md) - Build interactive AI agents with multiple modes, persistent threads, tool approval, and model switching using the Harness class.
- [Session](references/docs-harness-session.md) - Understand the Session — the live state object on a Harness — and the identity, thread, mode, grants, run, and display state it tracks.
- [Subagents](references/docs-harness-subagents.md) - Spawn focused child agents from a parent conversation with constrained tools and isolated or forked threads.
- [Threads and state](references/docs-harness-threads-and-state.md) - Manage persistent conversation threads and shared state across agent modes in a Harness.
- [Tool approvals and permissions](references/docs-harness-tool-approvals.md) - Gate tool execution with user approval flows and configure permission policies per-category or per-tool.
...
- [Reference: Harness class](references/reference-harness-harness-class.md) - API reference for the Harness class, the orchestrator for multi-mode agents with shared state, threads, and tool approvals.
- [Reference: Session class](references/reference-harness-session.md) - API reference for the Harness Session, which owns the per-conversation state for a single conversation: identity, threads, mode, model, run state, grants, and the display snapshot.
