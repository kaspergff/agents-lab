import { Workspace, Mastra, Agent, taskCheckTool, taskCompleteTool, taskUpdateTool, taskWriteTool, submitPlanTool, askUserTool, getServerSideFallbackInfo, createWorkspaceTools, summarizeTaskCheck } from '../chunk-HQPHHGZE.js';
export { askUserTool, assignTaskIds, submitPlanTool, taskCheckTool, taskCompleteTool, taskUpdateTool, taskWriteTool } from '../chunk-HQPHHGZE.js';
import { safeStringify } from '../chunk-RQL546GD.js';
import { createTool } from '../chunk-GR3BVMWK.js';
...
var SessionThread = class {
  ...
  async list(options) {
    ...
    return this.#store.listThreads({
      resourceId: options?.allResources ? void 0 : this.#getResourceId(),
...
var SessionState = class {
  ...
  #schema;
  #emit;
  constructor({ initialState, stateSchema, emit }) {
  ...
  async apply(updates) {
    ...
    this.#emit?.({ type: "state_changed", state: this.get(), changedKeys });
  }
  set(updates) {
...
var Session = class {
  ...
  model = new SessionModel(() => this.#store);
  /** The session's currently-selected mode and switch sequence. */
  mode = new SessionMode(() => this.#store, this.model);
  ...
  constructor({ resourceId, state }) {
    ...
    this.state = new SessionState(state ?? { initialState: {} });
  }
  /**
...
var Harness = class {
  ...
  constructor(config) {
    ...
    const initialModelId = config.initialState?.currentModelId;
    if (initialModelId) {
  ...
  createThreadDataStore() {
    return {
      listThreads: ({ resourceId, includeForkedSubagents }) => this.queryThreads({ resourceId, includeForkedSubagents }),
  ...
  async readThreadMetadataValue({ threadId, key }) {
    if (!this.config.storage) return void 0;
    try {
  ...
  async queryThreads({
    ...
    if (!this.config.storage) return [];
    const memoryStorage = await this.getMemoryStorage();
    const filter = resourceId === void 0 ? void 0 : { resourceId };
  ...
  async switchMode({ modeId }) {
    ...
    if (modelId) {
      this.emit({ type: "model_changed", modelId });
    }
  }
  ...
  async createThread({ title } = {}) {
    ...
    const projectPath = this.#session.state.get().projectPath;
    if (projectPath) {
  ...
  get memory() {
    return {
      ...
      listThreads: (options) => this.#session.thread.list(options),
  ...
  async loadThreadMetadata() {
    ...
    try {
      ...
      if (meta?.[modeModelKey2]) {
        this.#session.model.set({ modelId: meta[modeModelKey2] });
      } else {
        ...
        if (currentMode?.defaultModelId) {
          ...
          this.#session.model.set({ modelId: meta.currentModelId });
        }
      }
   ...
   */
  getObserverModelId() {
    return this.#session.state.get().observerModelId ?? this.config.omConfig?.defaultObserverModelId;
  }
  /**
   * Returns the reflector model ID from state, falling back to omConfig defaults.
   */
  getReflectorModelId() {
    return this.#session.state.get().reflectorModelId ?? this.config.omConfig?.defaultReflectorModelId;
  }
  /**
   * Returns the observation threshold from state, falling back to omConfig defaults.
   */
  getObservationThreshold() {
    return this.#session.state.get().observationThreshold ?? this.config.omConfig?.defaultObservationThreshold;
  }
  /**
   * Returns the reflection threshold from state, falling back to omConfig defaults.
   */
  getReflectionThreshold() {
    return this.#session.state.get().reflectionThreshold ?? this.config.omConfig?.defaultReflectionThreshold;
  }
  /**
...
${fence}` };
  ...
  buildSharedRunOptions() {
    const isYolo = this.#session.state.get().yolo === true;
    const shared = {
      ...
      for await (const chunk of subscription.stream) {
        ...
        try {
          ...
          if (streamResult || chunk.type === "finish" || chunk.type === "error" || chunk.type === "abort" || chunk.type === "tool-call-suspended") {
            const finishedRunId = chunkRunId ?? this.#session.run.getRunId();
    ...
    switch (chunk.type) {
      ...
      case "step-finish": {
        ...
        if (usage) {
          ...
          addOptionalUsageField2(stepUsage, "reasoningTokens", getUsageNumber(usageRecord, "reasoningTokens"));
          addOptionalUsageField2(stepUsage, "cachedInputTokens", getUsageNumber(usageRecord, "cachedInputTokens"));
          addOptionalUsageField2(
            stepUsage,
  ...
  async handleToolApprove({
    ...
    await agent.approveToolCall({
      ...
      toolCallId,
      requireToolApproval: !isYolo,
      memory: threadId ? { thread: threadId, resource: this.#session.identity.getResourceId() } : void 0,
      abortSignal: this.#session.run.ensureAbortController().signal,
      requestContext,
...
export { Harness, Session, defaultDisplayState, defaultOMProgressState, parseSubagentMeta };
