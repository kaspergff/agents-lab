...
var CoreToolBuilder = class extends chunkWSD4JNMB_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunkRS7FSLKM_cjs.isVercelTool(this.originalTool) && !chunkRS7FSLKM_cjs.isProviderDefinedTool(this.originalTool)) {
        ...
        if (chunkCMKSC37Z_cjs.isZodObject(schema) && isZodV4Schema(schema)) {
          ...
          if (isBackgroundEligible) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
              _background: chunk23FX5JPX_cjs.backgroundOverrideZodSchema
            });
          }
          if (isResumableTool) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
          ...
          if (jsonSchema2 && typeof jsonSchema2 === "object" && jsonSchema2.type === "object") {
            ...
            const injectedKeys = [];
            if (isBackgroundEligible) {
              properties._background = chunk23FX5JPX_cjs.backgroundOverrideJsonSchema;
  ...
  getParameters = () => {
    if (chunkRS7FSLKM_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType) {
    ...
    const mcpMeta = !chunkRS7FSLKM_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
    const execFunction = async (args, execOptions, toolSpan) => {
      try {
        let result;
        let suspendData = null;
        if (chunkRS7FSLKM_cjs.isVercelTool(tool)) {
          ...
          if (resumeData) {
            const resumeValidation = chunkRS7FSLKM_cjs.validateToolInput(resumeSchema, resumeData, options.name);
        ...
        if (suspendData) {
          ...
          const suspendValidation = chunkRS7FSLKM_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkRS7FSLKM_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkRS7FSLKM_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      try {
        ...
        const { data, error } = chunkRS7FSLKM_cjs.validateToolInput(parameters, args, options.name);
  ...
  build() {
    ...
    if (chunkRS7FSLKM_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
    ...
    const instanceNeedsApprovalFn = chunkRS7FSLKM_cjs.getNeedsApprovalFn(this.originalTool);
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkRS7FSLKM_cjs.Tool) && !chunkRS7FSLKM_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-ER5YO3AZ.cjs.map
//# sourceMappingURL=chunk-ER5YO3AZ.cjs.map