...
function messagesToPrompt(messages, options) {
  ...
  const converted = chunk2TATDSHU_cjs.convertMessages(messages).to("AIV5.Model");
  const lines = [];
...
function createResponseMessages(text, memoryInfo = {}) {
  ...
  return new chunk2TATDSHU_cjs.MessageList(memoryInfo).add(
    {
      role: "assistant",
      content: text
    },
...
var A2AAgent = class {
  ...
  async #consumeA2AStream({
    ...
    const messageList = new chunk2TATDSHU_cjs.MessageList({ threadId, resourceId });
