...
import { SpanType, TracingEventType, generateSignalId, DEFAULT_BLOCKED_LABELS, noOpLoggerContext, InternalSpans, EntityType } from '@mastra/core/observability';
      ...
      if (Array.isArray(val)) {
        ...
        for (let i = 0; i < Math.min(val.length, maxArrayLength); i++) {
          try {
            ...
            cleaned2.push(formatSerializationError(error));
          }
        }
...
var functionSchema = z.custom((value) => typeof value === "function", {
...
var serializationOptionsSchema = z.object({
...
var cardinalityConfigSchema = z.object({
...
var spanFilterSchema = functionSchema.optional();
...
var observabilityConfigValueSchema = z.object(observabilityInstanceConfigFields).refine(
  (data) => {
    const hasExporters = data.exporters && data.exporters.length > 0;
    const hasBridge = !!data.bridge;
    return hasExporters || hasBridge;
...
var sensitiveDataFilterOptionsSchema = z.object({
...
var LoggerContextImpl = class {
   ...
   */
  constructor(config) {
    const correlationContext = config.correlationContext ? { ...config.correlationContext } : void 0;
    this.config = {
      ...config,
...
var MetricsContextImpl = class {
   ...
   */
  constructor(config) {
    this.correlationContext = config.correlationContext ? { ...config.correlationContext } : void 0;
    this.traceId = config.traceId ?? this.correlationContext?.traceId;
    this.spanId = config.spanId ?? this.correlationContext?.spanId;
    this.metadata = config.metadata ? structuredClone(config.metadata) : void 0;
...
// src/client/id.ts
function generateClientSignalId() {
...
function nanosToDate(value) {
  ...
  if (typeof value === "string") {
    if (value.length <= 6) return /* @__PURE__ */ new Date(0);
    const ms = Number(value.slice(0, -6));
    return Number.isFinite(ms) ? new Date(ms) : /* @__PURE__ */ new Date(0);
  }
  return /* @__PURE__ */ new Date(0);
}
function flattenAttributes(otlpAttrs) {
  if (!Array.isArray(otlpAttrs)) return void 0;
  const out = {};
...
function decodeResourceSpans(payload) {
  const out = [];
  ...
  for (const rs of resourceSpans) {
    ...
    for (const ss of scopeSpans) {
      ...
      for (const span of spans) {
        ...
        const s = span;
        const traceId = typeof s.traceId === "string" ? s.traceId : void 0;
        const spanId = typeof s.spanId === "string" ? s.spanId : void 0;
        const name = typeof s.name === "string" ? s.name : void 0;
        if (!traceId || !spanId || !name) continue;
        const decoded = {
          traceId,
          spanId,
          parentSpanId: typeof s.parentSpanId === "string" && s.parentSpanId ? s.parentSpanId : void 0,
          name,
          startTime: nanosToDate(s.startTimeUnixNano),
          endTime: s.endTimeUnixNano !== void 0 ? nanosToDate(s.endTimeUnixNano) : void 0,
...
function decodeResourceLogs(payload) {
  const out = [];
  ...
  for (const rl of resourceLogs) {
    ...
    for (const sl of scopeLogs) {
      ...
      for (const record of logRecords) {
        ...
        const r = record;
        const traceId = typeof r.traceId === "string" ? r.traceId : void 0;
        if (!traceId) continue;
        let body = r.body;
        ...
        out.push({
          traceId,
          spanId: typeof r.spanId === "string" && r.spanId ? r.spanId : void 0,
          timestamp: r.timeUnixNano !== void 0 ? nanosToDate(r.timeUnixNano) : nanosToDate(r.observedTimeUnixNano),
          severityText: typeof r.severityText === "string" ? r.severityText : void 0,
          severityNumber: typeof r.severityNumber === "number" ? r.severityNumber : void 0,
          body,
...
function otlpSeverityToLogLevel(text, num) {
  if (text) {
...
function buildExportedSpan(decoded, options = {}) {
  const errorInfo = decoded.statusCode === 2 ? {
...
function buildExportedLog(decoded) {
  return {
    ...
    message: typeof decoded.body === "string" ? decoded.body : decoded.body !== void 0 ? String(decoded.body) : "",
...
var ClientObservabilityProxyImpl = class {
  #resolveInstance;
  #logger;
  #limits;
  constructor(options) {
  ...
  receive(payload, parentContext) {
    if (!payload || !payload.spans && !payload.logs && payload.executionDurationMs === void 0) {
      return;
    }
    let payloadBytes = 0;
    try {
    ...
    for (const decoded of decodedSpans) {
      ...
      const startedEvent = { type: TracingEventType.SPAN_STARTED, exportedSpan: exported };
      instance.__receiveExternalEvent(startedEvent);
      if (exported.endTime) {
        const endedEvent = { type: TracingEventType.SPAN_ENDED, exportedSpan: exported };
        instance.__receiveExternalEvent(endedEvent);
      }
    }
    for (const decoded of decodedLogs) {
      const log = buildExportedLog(decoded);
      const event = { type: "log", log };
...
function validateTraceIds(spans, logs, expected) {
  for (const s of spans) if (s.traceId !== expected) return false;
  for (const l of logs) if (l.traceId !== expected) return false;
  return true;
}
function validateParentLinks(spans, rootParent) {
  if (spans.length === 0) return true;
  const known = /* @__PURE__ */ new Set([rootParent]);
...
function createClientObservabilityProxy(options) {
...
var AUTH_FAILURE_STATUSES = /* @__PURE__ */ new Set([401, 403]);
var AUTH_COOLDOWN_BASE_MS = 6e4;
var AUTH_COOLDOWN_MAX_MS = 15 * 6e4;
var AUTH_COOLDOWN_JITTER_RATIO = 0.1;
var AuthFailureError = class extends Error {
  status;
...
async function fetchWithAuthFailureHandling(url, options, maxRetries) {
  ...
  try {
    ...
    if (authFailureStatus !== void 0) {
      throw new AuthFailureError(authFailureStatus, error);
    }
    throw error;
  }
}
var AuthFailureCooldown = class {
  constructor(exporterName, getLogger) {
  ...
  dropEventIfCoolingDown() {
    return this.dropEventsIfCoolingDown(1);
  }
  dropEventsIfCoolingDown(count) {
  ...
  recordFailure(args) {
    ...
    const cooldownSeconds = Math.ceil(cooldownMs / 1e3);
...
var CloudExporter = class extends BaseExporter {
  ...
  constructor(config = {}) {
    super(config);
    if (config.projectId !== void 0 && !VALID_PROJECT_ID.test(config.projectId)) {
  ...
  async flushBuffer() {
    ...
    const authFailure = results.find((result) => result.authFailureStatus !== void 0);
    ...
    if (authFailure?.authFailureStatus !== void 0) {
...
function resolveTracingStorageStrategy(config, observabilityStorage, storageName, logger) {
...
var DefaultExporter = class extends BaseExporter {
  ...
  emitDrop(signal, reason, count, error) {
    if (count === 0) return;
    const dropEvent = {
      type: "drop",
      signal,
      ...
      ...error === void 0 ? {} : { error: this.sanitizeDropError(error) }
    };
  ...
  async flushCreates(signal, events, storageCall) {
    ...
    try {
      ...
      if (error instanceof MastraError && error.domain === ErrorDomain.MASTRA_OBSERVABILITY && error.id.endsWith("_NOT_IMPLEMENTED")) {
...
function resolveBaseEndpoint2(baseEndpoint) {
  ...
  try {
    ...
    throw createInvalidEndpointError2(baseEndpoint, invalidText, error);
  }
}
...
function resolveExplicitSignalEndpoint2(signal, endpoint, projectId) {
  ...
  try {
    ...
    throw createInvalidEndpointError2(endpoint, invalidText, error);
  }
}
function deriveSignalEndpointFromTracesEndpoint2(signal, tracesEndpoint) {
  ...
  try {
    ...
    throw createInvalidEndpointError2(tracesEndpoint, invalidText, error);
  }
}
var MastraPlatformExporter = class extends BaseExporter {
  ...
  constructor(config = {}) {
    super(config);
    if (config.projectId !== void 0 && !VALID_PROJECT_ID2.test(config.projectId)) {
      throw createInvalidProjectIdError2(config.projectId);
    }
    const accessToken = config.accessToken || process.env.MASTRA_PLATFORM_ACCESS_TOKEN || process.env.MASTRA_CLOUD_ACCESS_TOKEN;
    ...
    if (tracesEndpointOverride) {
      ...
      baseEndpoint = resolveBaseEndpoint2(config.endpoint ?? "https://observability.mastra.ai");
...
function resolveTracingStorageStrategy2(config, observabilityStorage, storageName, logger) {
  const observabilityStrategy = observabilityStorage.observabilityStrategy;
  if (config.strategy && config.strategy !== "auto") {
    if (observabilityStrategy.supported.includes(config.strategy)) {
      return config.strategy;
...
function normalizeErrorInfo(error) {
  ...
  return {
    message: error.message,
    id: "id" in error && typeof error.id === "string" ? error.id : void 0,
    name: "name" in error && typeof error.name === "string" ? error.name : void 0,
    stack: "stack" in error && typeof error.stack === "string" ? error.stack : void 0,
    domain: "domain" in error && typeof error.domain === "string" ? error.domain : void 0,
    category: "category" in error && typeof error.category === "string" ? error.category : void 0,
    details: "details" in error && error.details && typeof error.details === "object" ? error.details : void 0
  };
}
